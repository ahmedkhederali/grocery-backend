...
const categoryCtrl = {
    ...
    updateProduct: async (req, res) => {
        try {
            ...
            const newChangeDate = [...product.changeDate,{
                date: new Date().toLocaleDateString('en-GB').replace(/\//g, '-'),
                value: req.body.price? req.body.price : product.price
            }]
            req.body.changeDate=newChangeDate
    ...
    updateProduct2: async (req, res) => {
        try {
            const { title, price, description,  images, category, changeDate } = req.body;
    ...
    specificProduct: async (req, res) => {
        try {
            const { id } = req.params
            const product = await Products.findById(id)
            if (!product) return res.status(500).json({ msg: "There Is No Product With This ID" })
